...
void kernel_calculate_density(LayerTilesGPU *d_hist, PointsPtr d_points, float dc, int numberOfPoints)
{
  auto queue = sycl::queue{Intel_gpu_selector{}};
  queue.submit([&](sycl::handler &cgh)
  {
...
void kernel_calculate_distanceToHigher(LayerTilesGPU *d_hist, PointsPtr d_points, float outlierDeltaFactor, float dc, int numberOfPoints)
{
  auto queue = sycl::queue{Intel_gpu_selector{}};
  queue.submit([&](sycl::handler &cgh)
  {
    cgh.parallel_for(sycl::range<1>(numberOfPoints), [=](sycl::id<1> idx)
    {
      ...
      if (i < numberOfPoints)
      {
        ...
        for (int xBin = search_box.x; xBin < search_box.y + 1; xBin++)
        {
          for (int yBin = search_box.z; yBin < search_box.y + 1; yBin++)
          {
            // get the id of this bin
            int binId = d_hist[layeri].getGlobalBinByBin(xBin, yBin);
            // get the size of this bin
